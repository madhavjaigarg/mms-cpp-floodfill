...
inline unsigned wallMask(Heading h){
    switch(h){case N:return WALL_N;case E:return WALL_E;case S:return WALL_S;default:return WALL_W;}
}
inline unsigned knownMask(Heading h){
    switch(h){case N:return KNOWN_N;case E:return KNOWN_E;case S:return KNOWN_S;default:return KNOWN_W;}
}
...
void showWall(int x,int y,Heading h){
    char d='n'; if(h==E)d='e'; else if(h==S)d='s'; else if(h==Wd)d='w';
    API::setWall(x,y,d);
}
...
bool senseRelative(Heading rel){
    switch(rel){
        ...
        case S: return API::wallFront(); // no back sensor, assume front
        case Wd: return API::wallLeft();
    }
    return false;
}
Heading relToAbs(Heading rel){return Heading((int(facing_) + int(rel)) & 3);} 

void senseAllSides(){
    bool wf = senseRelative(N);
    bool wr = senseRelative(E);
    bool wb = false; 
    bool wl = senseRelative(Wd);
...
void stepForward(){
    API::moveForward();
    if(facing_==N) y_++; else if(facing_==E) x_++; else if(facing_==S) y_--; else x_--;
    if(!inBounds(x_,y_)) { std::cerr << "Out of bounds: (" << x_ << "," << y_ << ")" << std::endl; std::terminate(); }
}
...
void recomputeDistances(){
    ...
    while(!q.empty()){
        int cx=q.front().first;int cy=q.front().second;q.pop();int cd=dist[cx][cy];
        static const int dx[4]={0,1,0,-1};
        static const int dy[4]={1,0,-1,0};
        static const Heading hh[4]={N,E,S,Wd};
        for(int k=0;k<4;k++){
            int nx=cx+dx[k];int ny=cy+dy[k];Heading h=hh[k];
...
bool chooseNextCell(int &tx,int &ty,Heading &th){
    int best=dist[x_][y_]; bool found=false; static const int dx[4]={0,1,0,-1}; static const int dy[4]={1,0,-1,0}; static const Heading hh[4]={N,E,S,Wd};
    for(int k=0;k<4;k++){int nx=x_+dx[k];int ny=y_+dy[k];Heading h=hh[k]; if(!inBounds(nx,ny)) continue; if(knowSide(x_,y_,h)&&haveWall(x_,y_,h)) continue; int d=dist[nx][ny]; if(d<best){best=d;tx=nx;ty=ny;th=h;found=true;}}
    return found;
}
...
void init(){
    ...
    int cx0=(mazeW-1)/2; int cx1=mazeW/2; int cy0=(mazeH-1)/2; int cy1=mazeH/2;
    goals.clear(); goals.push_back(std::make_pair(cx0,cy0)); if(cx1!=cx0||cy1!=cy0){goals.push_back(std::make_pair(cx1,cy0));goals.push_back(std::make_pair(cx0,cy1));goals.push_back(std::make_pair(cx1,cy1));}
    recomputeDistances(); senseAllSides(); recomputeDistances();
}

void solve(){init(); while(true){ if(atGoal()){std::cerr<<"Reached goal at ("<<x_<<","<<y_<<")!"<<std::endl; break;} int tx=x_,ty=y_; Heading th=facing_; if(!chooseNextCell(tx,ty,th)){std::cerr<<"No path!"<<std::endl; break;} face(th); stepForward(); senseAllSides(); recomputeDistances(); }}
